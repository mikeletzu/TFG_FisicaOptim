...
\section{Recurrencia}
Para analizar la importancia de la memoria recurrente de los modelos, en los casos en los que la simulación cuenta con inercia, se han comparado dos tipos de modelos, un MLP y un RNN. Los siguientes parámetros son compartidos entre ambas pruebas, con la intención de aislar únicamente la recurrencia y analizar su efecto.
\begin{table}[H]
    \begin{center}
...
RESULTADOS POR DEFINIR. En principio el RNN ayuda a que la tela mantenga su estructura aun que tiene un costo añadiido de guardar el history buffer. La diferencia de rendimiento no compensa la visual.
...
\begin{table}[H]
    \begin{center}
        ...
        \hline \textbf{Prueba} & \textbf{Tiempo} & \textbf{Error medio por vertice} & \textbf{FPS} & \textbf{Percepción} \\ \hline
            MLP & 0h(gpu) & 0.0 & 0 & 0/10 \\ \hline
            RNN & 0h(gpu) & 0.0 & 0 & 0/10 \\ \hline
        \end{tabular}
    \end{center}
...
\section{Velocidad}
Al utilizar secuencias de frames para el entrenamiento, teniendo las posiciones de cada vertice, la velocidad puede ser inferida. Sin embargo, la secuencia de las velocidad aporta al modelo de una conciencia acerca de la aceleración/decelarión de los vértices, por lo que en este experimento se valora su efecto.
