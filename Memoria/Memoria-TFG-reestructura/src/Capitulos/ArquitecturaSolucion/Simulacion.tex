...
SOBRA? Durante parte del desarollo del modelo, para poder identificar los motivos de inestibilidades e identificar otros errores con más facilidad, se redujo el tamaño de la malla a 6 vértices. \newline
...
\begin{itemize}
    \item \textbf{Posición}: Las posiciones de los vértices de la tela son los parámetros básicos de las investigaciones en este campo. Estas posiciones, se verá más adelante, pueden darse en espacio local del objeto o global.
    \item \textbf{SDF}: El Signed Distance Field es una magnitud que representará la distancia de cada vértice al objeto colisionador, llegando a ser 0 si toca la superfície, y un valor negativo si alcanza a traspasar. Se ha creado la clase estática \textit{SDFUtil} para habilitar los cálculos del SDF para conjuntos de \textit{colliders}. El script \textit{SDFUtil} contiene métodos que proporcionan el SDF de un punto a una esfera, una cápsula o dos listas de los dos tipos. También acepta un \textit{Transform} de un objeto de referencia si la posición del punto de origen no es global.
