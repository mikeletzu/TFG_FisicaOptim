...
Empresas pioneras en hardware y software han desarrollado tecnologías para mejorar el rendimiento gráfico incorporando esta clase de algoritmos. Entre ellas destacan: NVIDIA con DLSS (Deep Learning Super Sampling), donde tenemos el Resident Evil Requiem que usa DLSS 4; y AMD con FSR (FidelityFX Super Resolution). Asimismo, varias compañías lo incorporan en el propio curso del juego simulando comportamientos y mecánicas como EA (FIFA), Embark Studios (Arc Raiders) o Ubisoft con sus múltiples lineas de investigación con la IA. En este contexto, la aplicación de modelos de aprendizaje automático en simulaciones físicas se presenta de forma prometedora.

\section{Motivación}
Las simulaciones físicas no solo se dan en sistemas de alto rendimiento con hardware especializado y gráficas dedicadas, sino que también deben poder ejecutarse en máquinas con recursos más limitados.
...
En motores de videojuegos como Unity, más utilizado por desarrolladores indie, el cálculo de físicas se realiza por defecto en la CPU, lo que puede convertirse rápidamente en un cuello de botella cuando se pasa a escenarios más complejos. Esto conduce al creador a simplificar modelos, reducir la calidad de las animaciones o limitar la interacción física, afectando al desarrollo del producto y la experiencia de usuario. Creemos que incorporar la IA puede ser clave para este reto técnico, permitiendo agilizar el proceso en máquinas de menor rendimiento.\newline

Frente a esta problemática, el uso de modelos de aprendizaje automático ofrece una alternativa para aproximar un comportamiento físico detallado con un menor coste computacional en tiempo de ejecución. Este enfoque implica una carga elevada al inicio de entrenamiento de modelos, compensado con una ejecución eficiente en tiempo real.
Cabe destacar que, si bien utilizar la IA supone sacrificar la precisión absoluta de las físicas, no queremos caer en la falacia de la inmersión \cite{salenc2004rules}. Nuestro objetivo es lograr simulaciones visualmente verosímiles que mantengan una percepción fiel de realismo, adoptando así una perspectiva cercana al diseño de videojuegos.
Por ello, se ha escogido Cloth para la simulación de telas como caso de estudio debido a que representa un equilibrio adecuado entre accesibilidad tecnológica, complejidad física e impacto visual. De esta manera, realizaremos un análisis tanto técnico como perceptivo.
