...
El entrenamiento de este modelo recalca ciertas necesidades, algunas más obvias como ampliar el tamaño de la red neuronal y otras menos evidentes que se recogen a continuación:
...
\begin{itemize}
    ...
    Al coger un punto de referencia del que medir los desplazamientos y no basarse en posicines generadas, el accuracy del modelo, calculado con la diferencia entre las posiciones reales y predichas, en las pruebas de python mejora notablemente. No obstante, no se consigue alcanzar una simulación en Unity tan fiel como las producidas por la siguiente técnica.
...
Tras el proceso de iteraciones descrito, se concluye que la red neuronal más óptima es una RNN con \textit{unfolding}, utilizando como función de pérdida la suma entre \textit{L1Loss} y el la pérdida de velocidad. El input se construye con el SDF, la posición y velocidad de los vértices normalizados con inyección de ruido y como output se obtienen los deltas de desplazamiento normalizados. Por simplicidad el modelo que sa ha decidido representar por pseudocódigo recoge únicamente la posición y sdf como parámetro de entrada.
...
\For{$epoch \gets 1$ \textbf{to} $\text{EPOCHS}$}
    ...
    \State Almacenar métricas medias del $epoch$ en el histórico
